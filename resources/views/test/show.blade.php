     ...
     <div class="form-outline mb-3" style="display:flex">
      ...
      <button type="submit"  class="btn btn-primary">Recherche</button>
            ...
            <div class="container mx-auto pb-3"> 
                <a class="btn btn-primary" href="{{ Route('tests.index') }}">Retour</a>
                @csrf
            </div>
        </div>
    </div>
...
<table class="table table-bordered text-center table-striped">
            ...
            <tbody>
                @foreach ($list as $numero)
                    <tr>
                        ...
                        <td>{{ $numero->number }}</td>
                        <td>{{ $numero->status }}</td>
                        <td><form class="d-flex  " action="{{ route('delete-tutorial2', [$numero->id])}}" method="Post">
    ...
    <div> </br></br></br></br>
        <strong class="pl-3">Aucun numéro</strong></br>
    </br></br></br>
    <div class="pl-3" >
        <a class="btn btn-primary" href="/tests/edit?id={{$contrat->id}}&numero={{$contrat->numero}}">Ajouter numero</a>
</div>
    </div>
@endif
</div>
