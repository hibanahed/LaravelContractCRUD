...
<h2 class="text-center pt-5 pb-2"><strong>Modifier Employee</strong></h2></br>

<div class="container mx-auto pt-2">
<div class="container border border-5 rounded p-4" style="width:15cm">
 
<form action="{{url('tests/updateEmployee')}}"method="post">
@csrf 
<div class="row " style="height: 60vh;">

                    <input name="id" type="hidden" value="{{$employee->id}}"  class="form-control"  />
...
<div class="mx-auto col-10 col-md-8">
                    <div class="form-outline mb-2 ">
                        <strong>Nom:</strong>
                        <input type="text" value="{{$employee->lastName}}" name="lastName" class="form-control" required/>
                    </div>
                </div>
                <div class="mx-auto col-10 col-md-8">
                    <div class="form-outline mb-2 ">
                        <strong>Numero:</strong>
                        <input type="number"value="{{$employee->number}}" name="number"class="form-control" required/>
                    </div>
                </div>
                <div class="mx-auto col-10 col-md-8">
                    <div class="form-outline mb-2 ">
                        ...
                        @foreach($departments as $d)
                            <option value='{{$d->DepartmentId}}'{{$employee->DepartmentId == $d->DepartmentId ? 'selected':''}} >{{$d->name}} </option>
                            @endforeach    
                        </select>
                    </div>
                </div>
